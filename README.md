...
La aplicación conserva `dI/dT` como resultado ZTC principal y muestra el método relativo como control de sensibilidad. La vista de Evolución permite elegir un único dispositivo, guardar el conjunto de campañas bajo una subcampaña normalizada, ver la progresión de `I ZTC` de ambos métodos y consultar la dispersión de todas las campañas seleccionadas en un dashboard de dos columnas. La comparación automática entre dispositivos vive dentro de Análisis ZTC y entrega una tabla y gráficos para revisión manual.
